...
func (a *api) SetProfile(profile *Profile) (err error) {
    if attrs := profile.GetAttrs(); len(attrs) > 0 {
        req := setProfileReq{UserId: profile.GetUserId()}
        ...
        if err = a.client.Post(serviceName, commandSetProfile, req, &types.ActionBaseResp{}); err != nil {
            return
        }
    }
...
func (a *api) GetProfiles(userIds []string, attrs []string) (profiles []*Profile, err error) {
    ...
    if err = a.client.Post(serviceName, commandGetProfiles, req, resp); err != nil {
        return
    }
