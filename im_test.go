...
func TestIm_Push_Push(t *testing.T) {
	...
	message.OfflinePush().SetPushFlag(enum.PushFlagYes)
	message.OfflinePush().SetExt(map[string]interface{}{
		"url": "http://www.tencent.com",
	})
	message.OfflinePush().SetAndroidExtAsHuaweiIntentParam(enum.HuaweiIntentParamIntent)
	message.OfflinePush().SetApnsBadgeMode(enum.BadgeModeNormal)
...
func TestIm_SNS_GetFriends(t *testing.T) {
	...
	friends, err := NewIM().SNS().GetFriends("assistant", userIds, []string{
		...
		sns.SNSAttrGroup,
		entity.StandardAttrNickname,
		entity.StandardAttrBirthday,
	})
	if err != nil {
		t.Error(err)
		return
...
func TestIm_Private_ImportMessage(t *testing.T) {
	...
	message.SetReceivers("test1")
	message.SetTimestamp(time.Now().Unix())
	message.SetSyncOtherMachine(enum.SyncOtherMachineYes)
	message.SetContent(types.MsgTextContent{
		Text: "Hello world",
	})
