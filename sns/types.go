...
type (
	...
	friendInfo struct {
		...
		Remark     string `json:"Remark"`     // （选填）加好友备注
		GroupName  string `json:"GroupName"`  // （选填）分组信息，添加好友时只允许设置一个分组，因此使用 String 类型即可
		AddWording string `json:"AddWording"` // （选填）好友关系时的附言信息
	}
	
	// 添加好友（请求）
	addFriendsReq struct {
		UserId        string       `json:"From_Account"`  // （必填）需要为该UserID添加好友
		Friends       []friendInfo `json:"AddFriendItem"` // （必填）好友结构体对象
		AddType       string       `json:"AddType"`       // （选填）加好友方式（默认双向加好友方式）Add_Type_Single：表示单向加好友；Add_Type_Both：表示双向加好友
		ForceAddFlags int          `json:"ForceAddFlags"` // （选填）管理员强制加好友标记：1表示强制加好友，0表示常规加好友方式
	}
	...
	importFriendsReq struct {
		...
		Friends []importFriend `json:"AddFriendItem"` // 好友结构体对象
	}
	...
	importFriend struct {
		...
		AddWording string          `json:"AddWording"`           // （选填）好友关系时的附言信息
		AddTime    int64           `json:"AddTime"`              // （选填）形成好友关系的时间
		RemarkTime int64           `json:"RemarkTime"`           // （选填）好友备注时间
	...
	updateFriendsReq struct {
		...
		Friends []updateFriend `json:"UpdateItem"`   // （必填）需要更新的好友对象数组
	}
	...
	deleteFriendsReq struct {
		...
		DeleteUserIds []string `json:"To_Account"`   // （必填）待删除的好友的 UserID 列表，单次请求的 To_Account 数不得超过1000
		DeleteType    string   `json:"DeleteType"`   // （选填）删除模式
	}
	...
	deleteAllFriendsReq struct {
		UserId     string `json:"From_Account"` // （必填）需要删除该UserID的好友
		DeleteType string `json:"DeleteType"`   // （选填）删除类型
	}
	
	// 校验好友（请求）
	checkFriendsReq struct {
		UserId       string   `json:"From_Account"` // （必填）需要校验该 UserID 的好友
		CheckUserIds []string `json:"To_Account"`   // （必填）请求校验的好友的 UserID 列表，单次请求的 To_Account 数不得超过1000
		CheckType    string   `json:"CheckType"`    // （必填）校验模式
	}
	...
	addBlacklistReq struct {
		...
		BlackUserIds []string `json:"To_Account"`   // （必填）待添加为黑名单的用户 UserID 列表，单次请求的 To_Account 数不得超过1000
	}
	...
	deleteBlacklistReq struct {
		...
		DeleteUserIds []string `json:"To_Account"`   // （必填）待删除的黑名单的 UserID 列表，单次请求的 To_Account 数不得超过1000
	}
	...
	checkBlacklistReq struct {
		...
		CheckUserIds []string `json:"To_Account"`   // （必填）待校验的黑名单的 UserID 列表，单次请求的 To_Account 数不得超过1000
		CheckType    string   `json:"CheckType"`    // （必填）校验模式
	}
	...
	checkBlacklistResp struct {
		...
		FailUserIds  []string      `json:"Fail_Account"`
		CheckResults []CheckResult `json:"BlackListCheckItem"`
	}
	...
	addGroupsReq struct {
		...
		GroupUserIds []string `json:"To_Account"`   // （必填）需要加入新增分组的好友的 UserID 列表
	}
	...
	getGroupsReq struct {
		...
		NeedFriend   string   `json:"NeedFriend"`   // （选填）是否需要拉取分组下的 User 列表
		GroupName    []string `json:"GroupName"`    // （选填）要拉取的分组名称
	}
