...
type (
    ...
    queryAccountsOnlineStatusResp struct {
        types.ActionBaseResp
        QueryResult []OnlineStatusResult `json:"QueryResult"` // 用户在线状态结构化信息
        ErrorList   []OnlineStatusError  `json:"ErrorList"`   // 状态查询失败的帐号列表，在此列表中的目标帐号，状态查询失败或目标帐号不存在。若状态全部查询成功，则 ErrorList 为空
    }
