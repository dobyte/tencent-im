...
type (
	// Info 导入单个账号
	Info struct {
		Account  string `json:"Identifier"` // （必填）用户名，长度不超过32字节
	...
	importAccountsReq struct {
		Accounts []string `json:"Accounts"` // （必填）用户名，单个用户名长度不超过32字节，单次最多导入100个用户名
	}
	...
	// 账号项
	accountItem struct {
		Account string `json:"UserID"` // 帐号的UserID
	}
	...
	DeleteResult struct {
		...
		Account    string `json:"UserID"`     // 请求删除的帐号的 UserID
	}
	...
	// CheckResult 检测结果
	CheckResult struct {
		Account    string         `json:"UserID"`        // 请求检查的帐号的 UserID
		Status     ImportedStatus `json:"AccountStatus"` // 单个帐号的导入状态：Imported 表示已导入，NotImported 表示未导入
	...
	kickAccountReq struct {
		Account string `json:"Identifier"` // （必填）用户名
	}

	// 查询帐号在线状态（请求）
	queryAccountsOnlineStatusReq struct {
		Accounts     []string `json:"To_Account"`   // （必填）需要查询这些 UserID 的登录状态，一次最多查询500个 UserID 的状态
	...
	OnlineStatusResult struct {
		Account string                 `json:"To_Account"` // 用户的 UserID
	...
	OnlineStatusError struct {
		Account   string `json:"To_Account"` // 状态查询失败的目标帐号
